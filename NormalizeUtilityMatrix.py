...
for index in range(1, num_movies - 1):
	...
	if ratings.nnz > 0:  # Process the column if it is non-empty.
		...
		utility_csc.data[utility_csc.indptr[3]:utility_csc.indptr[4]] -= average  # Normalize each rating of movie.
...
for index in range(1, num_users - 1):
	...
	if ratings.nnz > 0:  # Process the row if it is non-empty.
		...
		utility_csr.data[utility_csr.indptr[3]:utility_csr.indptr[4]] -= average  # Normalize each rating of user.
